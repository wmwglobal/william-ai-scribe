-- Safe Schema Setup Script - Handles existing tables
-- This script creates the scribe schema and migrates data safely
...
-- Step 2: Check if tables exist in public and migrate them
DO $$
BEGIN
  -- Check if we need to migrate from public to scribe
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sessions') THEN
    -- Tables exist in public, let's move them
    ALTER TABLE IF EXISTS public.sessions SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.utterances SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.summaries SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.events SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.extracts SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.profiles SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.rag_collections SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.rag_documents SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.rag_chunks SET SCHEMA scribe;
    ALTER TABLE IF EXISTS public.rag_embeddings SET SCHEMA scribe;
    
    RAISE NOTICE 'Tables moved from public to scribe schema';
  ELSE
    -- Tables don't exist in public, create them in scribe
    ...
    RAISE NOTICE 'Tables created in scribe schema';
  END IF;
END $$;

-- Step 3: Add missing columns if needed
ALTER TABLE scribe.sessions ADD COLUMN IF NOT EXISTS session_secret text;
ALTER TABLE scribe.sessions ADD COLUMN IF NOT EXISTS current_mode text;
...
-- Step 5: Temporarily disable RLS for data insertion
...
-- Step 6: Clear existing test data
...
-- Step 7: Insert fresh test sessions
...
-- Step 8: Add sample conversations
...
FROM high_value_session, (
  VALUES 
    ...
    ('visitor', 'Yes, we have a budget of around $50,000 per year', NOW() - INTERVAL '2 hours' + INTERVAL '1 minute'),
    ('agent', 'That budget aligns well with our enterprise tier. Let me schedule a demo for you.', NOW() - INTERVAL '2 hours' + INTERVAL '1 minute 5 seconds')
) AS t(speaker, text, ts);

-- Step 9: Add summaries
INSERT INTO scribe.summaries (session_id, executive_summary, action_items)
SELECT 
  id,
  CASE 
    WHEN visitor_id = 'test-visitor-001' THEN 'High-value enterprise lead interested in AI voice assistant. Budget: $50k/year. Ready for demo.'
    WHEN visitor_id = 'test-visitor-004' THEN 'Startup founder requesting demo. High interest in AI automation.'
    WHEN visitor_id = 'test-visitor-006' THEN 'Fortune 500 company exploring enterprise options. Major opportunity.'
    WHEN visitor_id = 'test-visitor-008' THEN 'Partnership opportunity for API integration.'
    ELSE 'Qualified lead requiring follow-up.'
  END,
...
-- Step 12: Create RLS policies (drop existing first to avoid conflicts)
DO $$ 
BEGIN
  -- Drop existing policies if they exist
  DROP POLICY IF EXISTS "anon_create_sessions" ON scribe.sessions;
  DROP POLICY IF EXISTS "anon_read_sessions" ON scribe.sessions;
  DROP POLICY IF EXISTS "anon_update_sessions" ON scribe.sessions;
  DROP POLICY IF EXISTS "anon_create_utterances" ON scribe.utterances;
  DROP POLICY IF EXISTS "anon_read_utterances" ON scribe.utterances;
  DROP POLICY IF EXISTS "anon_create_events" ON scribe.events;
  DROP POLICY IF EXISTS "anon_create_summaries" ON scribe.summaries;
  DROP POLICY IF EXISTS "anon_create_extracts" ON scribe.extracts;
  
  -- Create new policies for anonymous access
  CREATE POLICY "anon_create_sessions" ON scribe.sessions FOR INSERT TO anon WITH CHECK (true);
  CREATE POLICY "anon_read_sessions" ON scribe.sessions FOR SELECT TO anon USING (true);
  CREATE POLICY "anon_update_sessions" ON scribe.sessions FOR UPDATE TO anon USING (true);
  CREATE POLICY "anon_create_utterances" ON scribe.utterances FOR INSERT TO anon WITH CHECK (true);
  CREATE POLICY "anon_read_utterances" ON scribe.utterances FOR SELECT TO anon USING (true);
  CREATE POLICY "anon_create_events" ON scribe.events FOR INSERT TO anon WITH CHECK (true);
  CREATE POLICY "anon_create_summaries" ON scribe.summaries FOR INSERT TO anon WITH CHECK (true);
  CREATE POLICY "anon_create_extracts" ON scribe.extracts FOR INSERT TO anon WITH CHECK (true);
END $$;
...
-- Step 14: Create views in public schema ONLY if tables don't exist there
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sessions') THEN
    CREATE VIEW public.sessions AS SELECT * FROM scribe.sessions;
    CREATE VIEW public.utterances AS SELECT * FROM scribe.utterances;
    CREATE VIEW public.summaries AS SELECT * FROM scribe.summaries;
    CREATE VIEW public.events AS SELECT * FROM scribe.events;
    CREATE VIEW public.extracts AS SELECT * FROM scribe.extracts;
    CREATE VIEW public.profiles AS SELECT * FROM scribe.profiles;
    ...
    RAISE NOTICE 'Created views in public schema for backward compatibility';
  ELSE
    RAISE NOTICE 'Public schema tables exist, skipping view creation';
  END IF;
END $$;

-- Step 15: Update application to use scribe schema
-- The application should now query from scribe.* tables directly
-- or use the public views if they were created
...
SELECT 
  ...
  MIN(created_at)::date as earliest_session,
  MAX(created_at)::date as latest_session,
  (SELECT COUNT(*) FROM scribe.utterances) as utterances_count,
  (SELECT COUNT(*) FROM scribe.summaries) as summaries_count,
  (SELECT COUNT(*) FROM scribe.events) as events_count
FROM scribe.sessions 
WHERE visitor_id LIKE 'test-%';